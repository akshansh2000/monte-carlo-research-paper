...
The Monte Carlo Method came into existence around the 1940s by scientists working on the atomic bomb. Named after Monaco, a city infamous for its casinos and games of ‘chances’, its purpose was to use random input samples to understand a complex system. 
...
The Statistician, based on the (F)Law of Averages, believes that the river is safe to cross, with its average depth of 3 ft (0.91 m). However, taking a numerical value as a substitute for an uncertainty does not constitute for the random variations. This is where the Monte Carlo Method steps in, providing accuracy over ‘single-point estimate analysis.

In such situations, it's possible to find probability distributions involving random variables using simulations.

Monte Carlo Methods / Experiments are algorithmic simulations which depend on repeating random sampling to get analytical results.[1] They allow significance in the risks of quantitative analysis. This concept uses randomness to find probability distributions.
...
\begin{figure}[h!]
    ...
    \vspace{10pt}
    \hspace{90pt}\scriptsize{\textbf{Figure 7}. \normalfont Simulating the Fractal Tree by increasing the depth}
\end{figure}
